  ...
  decs.forEach((dec, i) => {
    ...
    dec.getDecoders().forEach((channel_decoder, j) => {
      ...
      channel_decoder.setConversionFactor(
        channel_closure.unit_formatter.getConversionScale(),
        channel_closure.unit_formatter.getConversionOffset()
      );
        ...
        (counter, data, samples, subchannels) => {
          for (let sample = 0; sample < samples; sample++) {
            if (channel_closure.counter_time_scale == 0) {
              ...
              let time =
                counter * channel_closure.counter_time_scale +
                sample * channel_closure.sample_time_scale;
              console.log("channel_closure.counter_time_scale != 0", time);
            }
            for (let sub = 0; sub < subchannels; sub++) {
              console.log(
                channel_closure.unit_formatter.FormatBare(
                  data[sample * subchannels + sub]
                )
              );
            }
          }
...
async function stream(devices: Device[]) {
  ...
  for (let i = 0; i < device_info_array.length; i++) {
    ...
    console.log(
      `Enabling ${device_info_array[i].stream_count} streams from ${device_info_array[i].serial_number}`
    );

    console.log("Transfer count: ", streaming_counts.transfer_count);

    devices[i].startStreamingPackets(
      ...
      (status, stream_data, packet_size, packet_count) => {
        if (status == 0) {
          for (let packet = 0; packet < packet_count; packet++) {
            device_info_array[i].decoder.decode(
              stream_data.slice(packet * packet_size)
            );
          }
        } else {
  ...
  for (let i = 0; i < devices.length; i++) {
    console.log(
      `Disabling ${device_info_array[i].stream_count} streams from ${device_info_array[i].serial_number}`
    );
    for (let j = 0; j < device_info_array[i].stream_count; j++) {
