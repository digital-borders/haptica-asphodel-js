import {
  CHANNEL_TYPE_COMPOSITE_STRAIN,
  CHANNEL_TYPE_FAST_STRAIN,
  CHANNEL_TYPE_LINEAR_ACCEL,
  CHANNEL_TYPE_PACKED_ACCEL,
  CHANNEL_TYPE_SLOW_ACCEL,
  CHANNEL_TYPE_SLOW_STRAIN,
  ChannelInfo,
  createDeviceDecoder,
  Device,
  getChannelTypeName,
  getLibraryBuildDate,
  getLibraryBuildInfo,
  getLibraryProtocalVersion,
  getLibraryProtocalVersionString,
  getUnitTypeName,
  StreamAndChannels,
  TCPDeinit,
  TCPFindDevices,
...
function printSupplyInfo(dev: Device) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    console.log(
      `unit type=${info.unit_type} (${getUnitTypeName(info.unit_type)})`
    );
...
function printDecoderInfo(device: Device) {
  ...
  let device_decoder = createDeviceDecoder(
    streamchannel,
    count.filler_bits,
    count.id_bits
  );
  console.log(
    `Device decoder: streams=${device_decoder.getStreams()}, id_byte_offset=${device_decoder.getIDByteOffset()}`
  );
  ...
  decoders.forEach((d, i) => {
    ...
    console.log(
      `id=${
        ids[i]
      }, counter_byte_offset=${d.getCounterByteOffset()}, channels=${d.getChannels()}`
    );
    ...
    d.getDecoders().forEach((cd, j) => {
      ...
      console.log(
        `channel_bit_offset=${cd.getChannelBitOffset()}, samples=${cd.getSamples()}, subchannels=${cd.getSubChannels()}`
      );
    });
  });
}
  ...
  for (let i = 0; i < cnt; i++) {
    ...
    if (
      ...
      for (let bridge_index = 0; bridge_index < bridge_count; bridge_index++) {
        let subchannel_index = channel.getStrainBridgeSubchannel(
          bridge_index
        ) as number;
        let values = channel.getStrainBridgeValues(bridge_index);

        console.log(
          `Bridge: ${bridge_index} (subchannel_index=${subchannel_index})`
        );
      ...
      console.log(
        `   X axis self test difference: min=${limits[0]}, max=${limits[1]}`
      );
      console.log(
        `   Y axis self test difference: min=${limits[2]}, max=${limits[3]}`
      );
      console.log(
        `   Z axis self test difference: min=${limits[4]}, max=${limits[5]}`
      );
    } else {
...
function printChannelInfo(dev: Device) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    console.log(
      `channel type: ${info.channel_type})} (${getChannelTypeName(
        info.channel_type
      )})`
    );
    console.log(
      `unit_type: ${info.unit_type} (${getUnitTypeName(info.unit_type)})`
    );
    console.log(
      `filler_bits: ${info.filler_bits}, data_bits: ${info.data_bits}`
    );
    console.log(
      `samples: ${info.samples}, bits_per_sample: ${info.bits_per_sample}`
    );
    console.log(
      `minimum: ${info.minimum}, maximum: ${info.maximum}, resolution: ${info.resolution}`
    );
    ...
    if (cb.available == 0) {
      ...
      console.log(
        `calibration base setting=${cb.calibration.base_setting_index}, calibration resolution setting=${cb.calibration.resolution_setting_index}`
      );
      console.log(
        `calibration scale=${cb.calibration.scale}, calibration offset=${cb.calibration.offset}`
      );
      console.log(
        `calibration minimum=${cb.calibration.minimum}, calibration max=${cb.calibration.maximum}`
      );
    }
  }
}
...
function printStreamInfo(dev: Device) {
  ...
  for (let i = 0; i < count.count; i++) {
    ...
    console.log(
      "channels: ",
      stream_channels.result.slice(0, stream_channels.length)
    );

    const format = dev.getStreamFormat(i);
    let fmt = format.getInfo();
    console.log(
      `filler_bits=${fmt.filler_bits}, counter_bits=${fmt.counter_bits}`
    );
...
async function printLedInfo(dev: Device) {
  ...
  for (let i = 0; i < count; i++) {
    console.log("   RGB: ", dev.getRGBValues(i));
  }
...
function printDeviceInfo(device: Device) {
  ...
  console.log(
    "   Max Incoming Param Len: ",
    device.getMaxIncomingParamLength()
  );
  console.log(
    "   Max Outgoing Param Len: ",
    device.getMaxOutgoingParamLength()
  );
