...
export type StreamDecoder = {
  ...
  getChannels: () => number;
  setLostPacketCallback: (
    callback: (current: number, last: number) => void
  ) => void;
...
export type Device = {
  ...
  doTranfer: (
    command: number,
    params: Uint8Array,
    call_back: (status: number, params: Uint8Array) => void
  ) => void;
  doTranferReset: (
    command: number,
    params: Uint8Array,
...
export const TCPFindDevicesFilter: (
  filter: TCPFilter,
  length: number
) => Device[] = asp.TCPFindDevicesFilter;
export const TCPPollDevices: (millis: number) => void = asp.TCPPollDevices;
export const TCPCreateDevice: (
  host: string,
  port: number,
  timeout: number,
  serial: string
) => Device = asp.TCPCreateDevice;

export const createChannelDecoder: (
  channel_info: ChannelInfo,
  channel_bit_offset: number
) => ChannelDecoder = asp.createChannelDecoder;
export const createDeviceDecoder: (
  stream_and_channels: StreamAndChannels[],
  filler_bits: number,
  id_bits: number
) => DeviceDecoder = asp.createDeviceDecoder;
export const createStreamDecoder: (
  stream_and_channels: StreamAndChannels,
  stream_bit_offset: number
) => StreamDecoder = asp.createStreamDecoder;
export const getStreamingCounts: (
  stream_and_channels: StreamAndChannels[],
...
export const getLibraryProtocalVersion: () => number =
  asp.getLibraryProtocalVersion;
export const getLibraryProtocalVersionString: () => string =
  asp.getLibraryProtocalVersionString;
...
export function deviceToString(
  ...
  const custom_enum_counts = custom_enum_counts_h.result.slice(
    0,
  ...
  for (let i = 0; i < supply_count; i++) {
    ...
    supply_results.push(
      supply_result == null
        ? null
        : [supply_result.measurement, supply_result.result]
    );
  }
  ...
  return JSON.stringify({
    board_info: [board_info.info, board_info.rev],
    ...
    radio_ctrl_vars:
      radio_ctrl_vars == null
        ? []
        : [...radio_ctrl_vars.result.slice(0, radio_ctrl_vars.length)],
  ...
  public async final(filename: string | null) {
    return new Promise((resolve, reject) => {
      ...
      this.stream.on("finish", () => {
        ...
        const file = fs.createWriteStream(filename + ".apd");
        input.pipe(compressor).pipe(file);

        file.on("finish", () => {
          console.log(
            `apd file written successfully: from ${this.path} to ${filename}.apd`
          );
          fs.unlink(this.path, () => {
